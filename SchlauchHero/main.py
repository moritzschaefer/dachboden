...
class SchlauchHero:
    ...
    level_speed = [150,100, 90, 80, 70, 60, 50, 40, 30, 20, 10, 5]
    ball_speed = 10
    width = (8,20)
    light = 0
    lights = [0, 1, 2, 3]
    ...
    def check_correct(self, command):
        ...
        if len(pressed) == 1:
            ...
            if correct >= 0:
                ...
                self.score += 1
                self.update_score()
                self.point_sequence(pressed[0], pos=correct)
            else:
                # Wrong
    ...
    def point_sequence(self, color, pos):
        
        # Remove the color width
        w = 0
        while w < PIXELS //5:
            if self.int_board[w+pos] != color:
                break
            else:
                self.int_board[w+pos] = -1
            w += 1

        # Send the remaining color as a ball in opposite direction
        for i in range(w):
            self.ball_board[i + pos] = color
    ...
    def merge_ball_and_board(self):
        for i in range(PIXELS):
            if self.ball_board[i] >= 0 and self.ball_on:
    ...
    def move_color(self):
        ...
        if self.cur_generation_width < -3:
            if randint(0,100) > 15:
    ...
    def step(self, command):
        ...
        if self.mode == "sending":
            ...
            if utime.ticks_ms() - self.button_time > 100:
                self.check_correct(command)

            # Check if time is ready to move the light
            if utime.ticks_ms() - self.time > self.level_speed[min(self.score//15,len(self.level_speed)-1)]:
                self.time = utime.ticks_ms()
                self.move_color()
                self.change = 1
